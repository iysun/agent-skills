...
所有操作均可**单手（左手）键盘完成**；原方向键等键位保留。**总览模式下**：`WASD`(或方向键)移动高亮（A/D 左右、W/S 上下跳行）、`空格`/`Enter` 进入该页、`V`/`O`/`Esc` 退出（无需鼠标）。

| 键 | 作用 | 键 | 作用 |
|----|------|----|------|
| `A` / `D`（或 `←` `→` `空格` / 双击左右半屏） | 上一页 / 下一页 | `V`（或 `O`） | 总览缩略图 |
| `Q` / `E`（或 `Home` / `End`） | 首页 / 末页 | `S` | 演讲备注 |
| `F` | 全屏 | `T` / `C` | 主题 / 主题色 |
